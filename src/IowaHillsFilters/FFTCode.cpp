...
double SingleFreqDFT(double *Samples, int N, double Omega)
{
 ...
 TwiddleR =  cos(Omega * CUDART_PI);
 TwiddleI = -sin(Omega * CUDART_PI);
 ...
 for(k=0; k<N; k++)
  {
   SumR += Samples[k] *  cos((double)k * Omega * CUDART_PI);
   SumI += Samples[k] * -sin((double)k * Omega * CUDART_PI);
  }
 */
...
double Goertzel(double *Samples, int N, double Omega)
{
 ...
 CosVal = 2.0 * cos(CUDART_PI * Omega);
 for (j=0; j<N; j++)
  {
...
void WindowData(double *Data, int N, TWindowType WindowType, double Alpha, double Beta, bool UnityGain)
{
 ...
 if(WindowType == wtKAISER)
  ...
  {
	  for (j = 0; j<M; j++)WinCoeff[j] = constraintSinc((double)(2 * j + 1 - M) / dM * CUDART_PI);
   for(j=0; j<M; j++)WinCoeff[j] = pow(WinCoeff[j], Beta);
  }

 else if(WindowType == wtSINE)  // Hanning if Beta = 2
  {
	  for (j = 0; j<M / 2; j++)WinCoeff[j] = sin((double)(j + 1) * CUDART_PI / dM);
...
double constraintSinc(double x)
{
