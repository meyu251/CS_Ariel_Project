...
namespace mexplus {
	...
	*/
	...
	public:
		...
		*/
		template <typename T>
		static void append(mxArray* array, mwIndex index,mwIndex targetSize, const T& value) {
			appendInternal<T>(array, index, targetSize, value);
		}
		...
		*/
		...
		void append(const std::string& field,
			...
			|| MxComplexType<T>::value, T>::type* value, const size_t& length, const size_t& offset, mwIndex index = 0, const size_t& M = 1) {
			MEXPLUS_ASSERT(isOwner(), "Must be an owner to set.");
			appendSetInternal<T>(array_, field, index, value, length, offset, M);
		}
		/** Struct element write accessor.
		* @param field field name of the struct array.
		* @param value value of the field.
		...
		*/
		template <typename T>
		void append(const std::string& field, const T& value, mwIndex index = 0,const size_t& M = 1) {
			MEXPLUS_ASSERT(isOwner(), "Must be an owner to set.");
			appendSetInternal<T>(array_, field, index, value,M);
		}
		...
		*/
		std::string fieldName(size_t index) const {
			const char* field = mxGetFieldNameByNumber(array_, static_cast<int>(index));
	...
	private:
		...
		*/
		...
		template <typename Container>
		static mxArray *appendfromInternal(mxArray* array, const typename std::enable_if<
			MxArithmeticCompound<Container>::value, Container>::type& value);
		

		template <typename T>
		static mxArray *appendfromInternal(mxArray* array, typename std::enable_if<
			MxArithmeticType<T>::value, T>::type* value, const size_t& length, const size_t& offset);
		...
		*/
		...
		template <typename T>
		static mxArray* appendfromInternal(mxArray *array, typename std::enable_if<
			MxComplexType<T>::value, T>::type* value,const size_t& length,const size_t& offset);
		...
		*/
		...
		template <typename T>
		static mxArray* appendfromInternal(mxArray *array, typename std::enable_if<
			MxLogicalType<T>::value, T>::type* value,const size_t& length,const size_t& offset);
		...
		*/
		...
		template <typename T>
		static void appendInternal(mxArray* array,
			mwIndex index, mwIndex targetSize,
			const typename std::enable_if<
			!std::is_compound<T>::value ||
			MxComplexType<T>::value, T>::type& value);

		template <typename T>
		static void appendSetInternal(mxArray* array,
			const std::string& field,
			mwIndex index,
			const typename std::enable_if<
			MxLogicalCompound<T>::value
			|| MxArithmeticCompound<T>::value
			|| MxComplexCompound<T>::value , T>::type& value,const size_t& M);
		...
		static void appendSetInternal(mxArray* array,
			...
			MxLogicalType<T>::value
			|| MxArithmeticType<T>::value
			|| MxComplexType<T>::value, T>::type* value, const size_t& length, const size_t& offset, const size_t& M);
		...
		*/
		...
		/**
		template<typename T> 
		static typename std::enable_if<MxLogicalType<T>::value,T>::type castDouble(const double& db) {
			return (db != 0);
		}

		template<typename T>
		static typename std::enable_if<!MxLogicalType<T>::value, T>::type castDouble(const double& db) {
			return static_cast<T>(db);
		}

		template<typename T,typename R> 
		static typename std::enable_if<MxArithmeticCompound<T>::value, T>::type::value_type castSmart(const R& r) {
			typedef typename T::value_type ValueType;
			ValueType t = static_cast<ValueType>(r);
			return t;
		}


		template<typename T, typename R>
		static typename std::enable_if<MxArithmeticType<T>::value, T>::type castSmart(const R& r) {
		...
		template<typename T, typename R>
		static typename std::enable_if<!MxLogicalType<T>::value && MxCharCompound<T>::value, char>::type castSmart(const R& r) {
			char t = static_cast<char>(r);
			return t;
		}
		*/
	...
	*/
	template <typename Container>
	mxArray *MxArray::appendfromInternal(mxArray* array,const typename std::enable_if<
		MxArithmeticCompound<Container>::value, Container>::type& value) {
		typedef typename Container::value_type ValueType;
		size_t nelements = 0;
		if (!array) {
			array = mxCreateNumericMatrix(1,
				value.size(),
				MxTypes<ValueType>::class_id,
				MxTypes<ValueType>::complexity);
		} else {
			...
			if (mxIsComplex(array)) {
				...
				auto realptr = mxGetPr(array);
				auto realnewnewptr = mxRealloc(realptr, nbytes);
				mxSetPr(array, realnewnewptr);
				realptr = mxGetPi(array);
				realnewnewptr = mxRealloc(realptr, nbytes);
				mxSetPi(array, realnewnewptr);
			} else {*/
			nelements = mxGetNumberOfElements(array);
			size_t nbytes = (nelements +value.size())*mxGetElementSize(array);
				auto realptr = mxGetData(array);
				auto realnewnewptr = mxRealloc(realptr, nbytes);
				mxSetData(array, realnewnewptr);
				size_t M = mxGetM(array);
				size_t N = (nelements + value.size()) / M;
				mxSetN(array, N);
			//}
		}
		auto vt = reinterpret_cast<ValueType*>(mxGetData(array)) + nelements;
		MEXPLUS_CHECK_NOTNULL(array);
		std::copy(value.begin(),
			value.end(),
			vt);
	...
	template <typename T>
	mxArray *MxArray::appendfromInternal(mxArray* array, typename std::enable_if<
		MxArithmeticType<T>::value, T>::type* value, const size_t& length, const size_t& offset) {
		size_t nelements = 0;
		if (!array) {
			array = mxCreateNumericMatrix(1,
			...
			nelements = mxGetNumberOfElements(array);
			size_t nbytes = (nelements + length)*mxGetElementSize(array);
			auto realptr = mxGetData(array);
			auto realnewnewptr = mxRealloc(realptr, nbytes);
			mxSetData(array, realnewnewptr);
			size_t M = mxGetM(array);
			size_t N = (nelements + length) / M;
			mxSetN(array, N);
		}
		T* dx = reinterpret_cast<T*>(mxGetData(array)) + nelements;
		for (size_t it = offset; it < offset + length; it++) {
			*dx++ = value[it];
		}
		return array;
	}
	...
	*/
	...
	mxArray* MxArray::appendfromInternal(mxArray *array, const typename std::enable_if<
		...
		ValueType* imag = reinterpret_cast<ValueType*>(mxGetPi(array));
		typename Container::const_iterator it;
		for (it = value.begin()+nelements; it != value.end(); it++) {
			*real++ = (*it).real();
			*imag++ = (*it).imag();
		}
		return array;
	...
	mxArray* MxArray::appendfromInternal(mxArray *array, typename std::enable_if<
		MxComplexType<T>::value, T>::type* value, const size_t& length, const size_t& offset) {
		typedef typename T ContainerValueType;
		typedef typename ContainerValueType::value_type ValueType;
		size_t nelements = 0;
		if (!array) {
			array = mxCreateNumericMatrix(1,
		...
		MEXPLUS_CHECK_NOTNULL(array);
		ValueType* real = reinterpret_cast<ValueType*>(mxGetPr(array))+nelements;
		ValueType* imag = reinterpret_cast<ValueType*>(mxGetPi(array))+nelements;
	...
	mxArray* MxArray::appendfromInternal(mxArray *array,const typename std::enable_if<
		...
		size_t nelements = 0;
		if (!array) {
			array = mxCreateLogicalMatrix(1, value.size());
		} else {
			nelements = mxGetNumberOfElements(array);
			size_t nbytes = (nelements + value.size())*mxGetElementSize(array);
			auto realptr = mxGetData(array);
		...
		MEXPLUS_CHECK_NOTNULL(array);
		auto vtl = mxGetLogicals(array) + nelements;
		std::copy(value.begin(), value.end(), vtl);
		return array;
	}
	...
	mxArray* MxArray::appendfromInternal(mxArray *array, typename std::enable_if<
		...
		size_t nelements = 0;
		if (!array) {
			array = mxCreateLogicalMatrix(1, length);
		} else {
			nelements = mxGetNumberOfElements(array);
			size_t nbytes = (nelements + length)*mxGetElementSize(array);
			auto realptr = mxGetData(array);
		...
		MEXPLUS_CHECK_NOTNULL(array);
		auto vtl = mxGetLogicals(array) + nelements;
		std::copy(value+offset, value+offset+length, stdext::checked_array_iterator<T*>(vtl,length));
		//for (size_t pos=)
		return array;
	}
	...
	*/
	template <typename T>
	void MxArray::appendInternal(mxArray* array,
		mwIndex index,mwIndex targetSize,
		const typename std::enable_if<
		!std::is_compound<T>::value ||
		MxComplexType<T>::value, T>::type& value) {
		MEXPLUS_CHECK_NOTNULL(array);
		MEXPLUS_ASSERT(static_cast<size_t>(index) < static_cast<size_t>(targetSize),
			"Index out of range: %u.",
			index);
		MEXPLUS_ASSERT(!mxIsCell(array), "Append cannot handle cells array...");
		if (static_cast<size_t>(targetSize) > mxGetNumberOfElements(array)) {
			size_t nbytes = targetSize * mxGetElementSize(array);
			if (mxIsComplex(array)) {
	...
	*/
	...
	template <typename T>
	void MxArray::appendSetInternal(mxArray* array,
		const std::string& field,
		mwIndex index,
		const typename std::enable_if<
		MxLogicalCompound<T>::value
		...
		mxArray *mxold = mxGetFieldByNumber(array, index, field_number);
		//mxArray* new_item = from(value);
		mxold = appendfromInternal<T>(mxold, value);
		size_t elements_num = mxGetNumberOfElements(mxold);
		size_t N = elements_num / M;
		mxSetM(mxold, M);
		mxSetN(mxold, N);
		//mxDestroyArray(mxGetFieldByNumber(array, index, field_number));
		mxSetFieldByNumber(array, index, field_number, mxold);
	}
	...
	void MxArray::appendSetInternal(mxArray* array,
		...
		mxArray *mxold = mxGetFieldByNumber(array, index, field_number);
		//mxArray* new_item = from(value);
		mxold = appendfromInternal<T>(mxold, value,length,offset);
		size_t elements_num = mxGetNumberOfElements(mxold);
		size_t N = elements_num / M;
		mxSetM(mxold, M);
		mxSetN(mxold, N);
