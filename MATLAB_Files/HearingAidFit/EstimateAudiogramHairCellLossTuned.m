%EstimateAudiogramHairCellLossTuned
clear all
close all
...
%load AudioMeasurements %Liberman Data
%Audiogram=Audio(51,:);
...
%Find Inner Hair Cell Loss

Gain0(sub,:)= CalculateJNDfiles2( TestFreq,noise,run_time,...
    En,testedPowerLevels,0.5, IHC_Vector)-JNDref;


OHCindex(sub)=OHC_Vector;
IHCindex(sub,:)=IHC_Vector;
Ethreshold(sub,:)=JND-JNDref';
Gain(sub,:)=min(Audiogram,Gain0(sub,:));

save TunedData TestFreq TunedAudiograms OHCindex IHCindex Ethreshold Gain
end
PlotTunedResults
